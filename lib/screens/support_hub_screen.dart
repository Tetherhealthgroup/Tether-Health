...
class SupportHubScreen extends StatelessWidget {
  ...
  ///
  /// Separate from [onCallQuitline], which predates the Spanish line and says
  /// nothing about which number was chosen. The host answers this with the
  /// same dialog the approved artwork used, so the number shown is the number
  /// copied.
  final void Function(QuitlineContact)? onQuitlineSelected;
  ...
  /// The quitline for the language on screen, and the other one.
  ///
  /// Both are reachable from this card, always. The approved artwork this
  /// screen replaced drew two numbers, and `fix/contact-routing-and-tap-targets`
  /// existed because only one of them could be reached — showing a Spanish
  /// speaker an English-only quitline is the defect that fix was for, and
  /// replacing the artwork with a widget must not reintroduce it.
  QuitlineContact get _quitline =>
      isSpanish ? ContactInfo.spanish : ContactInfo.english;

  QuitlineContact get _otherQuitline =>
      isSpanish ? ContactInfo.english : ContactInfo.spanish;
  ...
  void _handleCallQuitline(BuildContext context) {
    ...
    _showQuitline(context, _quitline);
  }

  /// The dialog for one quitline. Named per line rather than generic, so the
  /// number in the title is the number that gets copied.
  void _showQuitline(BuildContext context, QuitlineContact quitline) {
    if (onQuitlineSelected != null) {
      onQuitlineSelected!(quitline);
      return;
    }
    _showInfo(
      context,
      'Call ${quitline.vanityNumber}?',
      t(
        'This free, confidential ${quitline.language} service routes the '
            'caller to their state quitline (${quitline.dialledNumber}). '
            'Services and hours may vary. Phone calling can be connected to '
            'the device dialer when the native calling integration is added.',
        'Este servicio gratuito y confidencial en ${quitline.language} te '
            'conecta con la línea de tu estado (${quitline.dialledNumber}). '
            'Los servicios y horarios pueden variar.',
      ),
    );
  }
                    ...
                    ResourceCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 12),
                          Semantics(
                            label: 'Call ${_quitline.vanityNumber}',
                            button: true,
                            container: true,
                            excludeSemantics: true,
                            child: GestureDetector(
                              onTap: () => _showQuitline(context, _quitline),
                              child: Text(
                                _quitline.dialledNumber,
                                style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 22,
                          ...
                          const SizedBox(height: 8),
                          // The other language's line, always present. See
                          // [_quitline].
                          Semantics(
                            label: 'Call ${_otherQuitline.vanityNumber}',
                            button: true,
                            container: true,
                            excludeSemantics: true,
                            child: GestureDetector(
                              key: const ValueKey('support-other-quitline'),
                              onTap: () =>
                                  _showQuitline(context, _otherQuitline),
                              // The vanity number names its own language —
                              // "DÉJELO-YA" is not ambiguous — and repeating
                              // the language word here would put it on screen
                              // twice once the dialog opens.
                              child: Text(
                                _otherQuitline.vanityNumber,
                                style: const TextStyle(
                                  color: AppColors.mintStrong,
                                  fontSize: 14,
